...
function PaymentHistory() {
  ...
  const fetchPayments = async () => {
    try {
      setLoading(true);
      // Sorting optimized for Date and then Creation Time
  ...
  // Helper for Payment Mode Styles
    ...
    <div className="p-4 md:p-8 space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-[24px] shadow-sm border border-slate-100">
        <div>
          ...
          <p className="text-slate-400 text-xs font-bold uppercase tracking-widest mt-1">Real-time payment history</p>
        </div>

        <div className="relative group overflow-hidden rounded-2xl border border-slate-200 focus-within:border-blue-500 transition-all shadow-sm">
          ...
          <input 
            ...
            className="pl-12 pr-6 py-4 w-full md:w-80 outline-none font-bold text-slate-700 placeholder:text-slate-300"
                ...
                <tr>
                  <td colSpan="4" className="py-20">
                    <div className="flex flex-col items-center justify-center gap-4">
                      ...
                      <span className="text-xs font-black text-slate-400 uppercase">Fetching Logs...</span>
                    </div>
                  </td>
                </tr>
                  ...
                  <tr key={p.id} className="hover:bg-slate-50/80 transition-all group">
                    <td className="px-8 py-5">
                      <div className="flex items-center gap-4">
                        <div className="w-10 h-10 rounded-xl bg-slate-900 text-white flex items-center justify-center text-xs font-black shadow-lg shadow-slate-200">
                          {p.members?.name?.substring(0, 2).toUpperCase() || '??'}
                        </div>
